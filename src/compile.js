...
console.log('compile cwd:',process.cwd())
...
const releaseFile = path.join(process.cwd(), 'release.json');
...
const compileAll = function anonymous(cb) {
  ...
  const files = [];
  fs.recurseSync(path.join(process.cwd(), 'src'), (filepath) => {
...
function compile() {
  ...
  if (watch) {
    ...
    const watcher = new gaze.Gaze(path.join(process.cwd(),'src/**/*'));
    // watch file change
